...
## 特性一览
- 多存储策略：本地、阿里云 OSS、腾讯云 COS（QCloud）、华为云 OBS、又拍云 Upyun、七牛云 Qiniu、WebDAV、123盘 OpenAPI、Sae、Ace
- 在线预览：文本/图片/音频/视频 文件在线预览
- 外链分享：直链、UBB/HTML 代码，便于论坛/博客嵌入
- 高级分享：链接签名（有效期/下载次数）、下载密码、仅登录可见、按 UID 白名单可见、批量生成高级分享链接
- 完整用户中心：文件列表、重命名/删除、批量重命名、批量高级分享、API Token 一键生成、第三方账号绑定（QQ/微信/Linux Do）、2FA 入口
- 开放 API：上传/列表/下载/签名分享等接口，内置 API 示例与文档（api-docs.php）
- 安全控制：IP 黑名单、图片违规检测、登录限制、可选隐藏用户 IP（脱敏）
...
2) 存储与直链
- 新增存储适配器：WebDAV、123盘 OpenAPI（Openapi123 驱动）
- 直链域名与协议重写：支持 `downfile_domain` 与 `downfile_protocol` 输出自定义直链
- 123OpenAPI 增强：可选客户端 API 模式、速率限制与熔断、元数据缓存、调试日志视图（view_123api_log.php）
...
4) 用户中心与分享
- 完整的用户中心：文件表格（分页/勾选）、重命名/删除、批量重命名、批量高级分享
- 高级分享：有效期（秒）、最大下载次数、下载密码、仅登录可见、按 UID 白名单可见；生成的签名链接支持直下与预览
