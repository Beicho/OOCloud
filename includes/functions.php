...
function mask_ip($ip){
	global $conf;
	// 如果配置了隐藏IP，则脱敏显示
	if($conf['hide_user_ip'] == '1'){
		$parts = explode('.', $ip);
		if(count($parts) == 4){
			// IPv4: 显示为 xxx.xxx.xxx.***
...
function checkRefererHost(){
	// 允许缺失 Referer（部分浏览器/隐私设置会去掉），优先用 CSRF 校验
...
function file_output($hash, $type, $size, $name, $is_view = false, $is_admin = false, $force_relay = false){
	global $conf, $stor;
    ...
    if(\lib\StorHelper::is_cloud() && $conf['downfile_type'] == 1){
        // 对 123 OpenAPI 驱动，name 传 cloud_id
        $passName = $hash;
        if($conf['storage']==='openapi123'){
            global $DB;
            $cid = $DB->getColumn("SELECT cloud_id FROM pre_file WHERE hash=:h LIMIT 1", [':h'=>$hash]);
            if(!empty($cid)) $passName = $cid;
        }
        // 如果强制中转模式，跳过云存储直链，直接使用服务器中转
        if(!$force_relay){
            $redirect = $stor->getDownUrl($passName, $name, $is_view ? minetype($type) : null);
            ...
            if($redirect){
                ...
                ob_clean();
                $msg = '获取下载链接失败';
                $arr = json_decode($errmsg, true);
                if(is_array($arr) && isset($arr['code'])){
                    $code = intval($arr['code']);
                    $m = isset($arr['message'])?$arr['message']:'';
                    // 根据不同错误码显示友好提示
                    if($code==5113){ $msg = '下载流量不足，请尝试使用"中转下载"按钮。'; }
                    elseif($code==1010){ $msg = '地域限制错误，请使用"中转下载"按钮或在后台启用"优先使用客户端API"。错误详情：'.$m; }
                    elseif($code==5066){ $msg = '文件不存在或已被删除。'; }
                    elseif(!empty($m)){ $msg = $m; }
                }
                sysmsg('<h3>'.$msg.'</h3>');
                exit('Error:'.$errmsg);
            }
