	...
	$("#listTable").bootstrapTable({
		...
		columns: [
			...
			},
            {
                ...
                title: '操作',
                formatter: function(value, row, index) {
                    var btns = '<a href="./file.php?uid='+row.uid+'" class="btn btn-xs btn-info" target="_blank">文件</a>';
                    btns += '&nbsp;<a href="javascript:delUser('+row.uid+')" class="btn btn-xs btn-danger">删除</a>';
                    if(row.rtype==='local'){
                        btns += '&nbsp;<a href="javascript:resetPwd('+row.uid+')" class="btn btn-xs btn-warning">改密</a>';
                        btns += '&nbsp;<a href="javascript:setTotp('+row.uid+')" class="btn btn-xs btn-default">2FA</a>';
                    }
                    return btns;
                }
            },
...
function setTotp(uid){
    layer.prompt({title:'设置2FA密钥（Base32）。留空为清除。', formType:0}, function(sec, idx){
        layer.close(idx);
        var ii = layer.load(2, {shade:[0.1,'#fff']});
        $.ajax({
            type:'POST', url:'ajax.php?act=setUserTotp', data:{uid:uid, secret:sec}, dataType:'json',
            success:function(d){ layer.close(ii); if(d.code==0){ layer.alert('设置成功',{icon:1}); } else { layer.alert(d.msg,{icon:2}); } },
...
function resetPwd(uid){
    layer.prompt({title:'输入新密码(>=6位)', formType:1}, function(pwd, idx){
        layer.close(idx);
        if(!pwd || pwd.length<6){ layer.alert('密码至少6位',{icon:7}); return; }
        var ii = layer.load(2, {shade:[0.1,'#fff']});
        $.ajax({
            type:'POST', url:'ajax.php?act=resetLocalPassword', data:{uid:uid, password:pwd}, dataType:'json',
            success:function(d){ layer.close(ii); if(d.code==0){ layer.alert('修改成功',{icon:1}); searchSubmit(); } else { layer.alert(d.msg,{icon:2}); } },
            error:function(){ layer.close(ii); layer.msg('服务器错误'); }
        });
    });
}
